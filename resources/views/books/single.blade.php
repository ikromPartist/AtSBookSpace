...
@section('content')
         ...
         <div class="container">
            <ul class="breadcrumbs__list">
               ...
               <li class="breadcrumbs__item">
                  ...
                  <a class="breadcrumbs__link" href="{{route('books.index')}}">Книги</a>
               </li>
            ...
            <section class="book-info">
               <div class="book-info__status-wrapper" data-book-status="{{$book->id}}">
                  @if (!$book->user && $book->queues_count == 0)
                     <p class="book-info__status"">Доступна</p>
                  @else
                     <p class="book-info__status books-info__status--unavailable">
                        Занято примерно до: 
                        @php
                           $date = null;
                           if ($book->return_date) {
                  ...
                  @endif
                  <p class="book-info__article">Артикул: {{$book->code}}</p>
               </div>{{-- status wrapper end --}}
               <p class="book-info__description">{{$book->description}}</p>
               <ul class="book-info__list">
                  <li class="book-info__item">
                     <div class="book-info__dt">Автор</div>
                     <div class="book-info__dd">
                        <span class="material-icons book-info__arrow-icon">arrow_forward_ios</span>
                        {{$book->author}}
                     </div>
                  </li>
                  <li class="book-info__item">
                     <div class="book-info__dt">Страницы</div>
                     <div class="book-info__dd">
                        <span class="material-icons book-info__arrow-icon">arrow_forward_ios</span>
                        {{$book->pages}}
                     </div>
               ...
               <div class="book-info__rating-wrapper">
                  <div class="book-info__stars-wrapper">
                     <span class="material-icons book-info__rating-icons">{{$book->rating >= 1 ? 'star' : 'star_outline'}}</span>
                     <span class="material-icons book-info__rating-icons">{{$book->rating >= 2 ? 'star' : 'star_outline'}}</span>
                     <span class="material-icons book-info__rating-icons">{{$book->rating >= 3 ? 'star' : 'star_outline'}}</span>
                     <span class="material-icons book-info__rating-icons">{{$book->rating >= 4 ? 'star' : 'star_outline'}}</span>
                     <span class="material-icons book-info__rating-icons">{{$book->rating >= 5 ? 'star' : 'star_outline'}}</span>
                  </div>
                  <button class="button book-info__rating-link" type="button" data-id="show-rating-modal">Оценить</button>
               </div>
               <div class="likes-container" data-id="likes-container">
                  ...
                  @endphp
                  @if ($liked)
                     <button class="likes-button" type="button" data-id="like-button" data-type="liked">
                        <span class="material-icons likes-icon">thumb_up</span>
                        <output>{{$book->likes_count}}</output>
                     </button>
                  @else
                     <button class="likes-button" type="button" data-id="like-button" data-type="not-liked" data-book="{{$book->id}}">
                        <span class="material-icons likes-icon likes-icon--empty">thumb_up</span>   
                        <output>{{$book->likes_count}}</output>
                     </button>
                  @endif
                  @php $disliked = false;
                  ...
                  @endphp
                  @if ($disliked)
                     <button class="likes-button" type="button" data-id="dislike-button" data-type="disliked">
                        <span class="material-icons likes-icon">thumb_down</span>
                        <output>{{$book->dislikes_count}}</output>
                     </button>
                  @else
                     <button class="likes-button" type="button" data-id="dislike-button" data-type="not-disliked" data-book="{{$book->id}}">
                        <span class="material-icons likes-icon likes-icon--empty">thumb_down</span>   
                        <output>{{$book->dislikes_count}}</output>
                     </button>
                  @endif  
               </div>{{--likes container end --}}
               <a class="button book-info__book-link" data-type="booking" data-book="{{$book->id}}" data-queue="{{$book->queues_count + 1}}" href="#">Забронировать</a>
            </section>{{-- book info end --}}
         </div>{{-- book info wrapper end --}}
         
         <section class="comment">
            <h2 class="comment__title">
               <output>{{$book->comments_count}}</output> Коментария
            </h2>
            <form class="comment-form">
               ...
               <div class="comment-form__top-wrapper">
                  ...
                  <textarea class="comment__text" data-id="comment-text" spellcheck="false" placeholder="Оставить комментарий..."></textarea>
               </div>
               <div class="comment__buttons-wrapper" data-id="buttons-wrapper">
                  <button class="button button--red" type="reset" data-id="cancel-comment">Отмена</button>
                  <button class="button" type="submit" data-id="submit-comment" data-book="{{$book->id}}">Комментировать</button>
               </div>
            </form>
            <ul class="comments">
               @foreach ($comments as $comment)
               <li class="comments__item">
                  <a class="comments__link" href="{{route('profile.single', $comment->user->id)}}">
   ...
   <section class="modal hidden" data-id="rating-modal">
      <div class="modal__container">
         ...
         <div class="modal__btn-wrapper">
            <button class="button button--red" data-id="rating__cancel-btn" type="button">Отмена</button>
         </div>
   ...
   <section class="modal modal--success hidden" data-id="rating-success">
      <div class="modal__container">
         <p class="modal__text">Спасибо за отзыв!</p>
         <div class="modal__btn-wrapper">
            <button class="button" data-id="rating__ok-btn" type="button">OK</button>
         </div>
         <button class="modal__close-btn" type="button">
            <span class="material-icons modal__close-icon" data-id="rating__close-btn">close</span>
         </button>
