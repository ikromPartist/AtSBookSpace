      ...
      <li class="booked-books__item">
         <figure class="books-card">
            <div class="books-card__image-wrapper">
               <img class="books-card__image" src="{{asset('img/books/' . $book->book->img)}}" alt="{{__('Название книги')}}">
               <div class="books-card__overlay">
                  <a class="books-card__links books-card__links--more" href="{{route('books.single', $book->book->id)}}">{{__('Подробнее')}}</a>
               </div>
            ...
            <h3 class="books-card__title">
               ...
               <b class="books-card__pages">{{$book->book->pages}} {{__('стр.')}}</b>
            </h3>
            <div class="books-card__rating-wrapper">
               <div>
                  <span class="material-icons-outlined books-card__rating-icons">{{$book->book->rating >= 1 ? 'star' : 'star_outline'}}</span>
                  <span class="material-icons-outlined books-card__rating-icons">{{$book->book->rating >= 2 ? 'star' : 'star_outline'}}</span>
                  <span class="material-icons-outlined books-card__rating-icons">{{$book->book->rating >= 3 ? 'star' : 'star_outline'}}</span>
                  <span class="material-icons-outlined books-card__rating-icons">{{$book->book->rating >= 4 ? 'star' : 'star_outline'}}</span>
                  <span class="material-icons-outlined books-card__rating-icons">{{$book->book->rating >= 5 ? 'star' : 'star_outline'}}</span>
               </div>
